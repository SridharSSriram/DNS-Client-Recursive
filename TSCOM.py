...
def server():
    ...
    print("[TS COM]: Server IP address is  ", host_ip)

    dns_com_map = create_dns_map()
    ...
    while True:
        ...
        if received_hostname is "PROJ2-HNS.txt EOF reached":
            break
        returned_tuple = []
        if dns_com_map.get(received_hostname) is None:
            ...
            returned_tuple.append(received_hostname)
            returned_tuple.append(dns_com_map[received_hostname][0])
            returned_tuple.append(dns_com_map[received_hostname][1])
...
def create_dns_map():
    ts_com_map = dict()
    ts_com_table_name = sys.argv[1]
    with open(ts_com_table_name) as ts_map:
        for line in ts_map:
            ...
            ts_com_map[key_hostname] = value_ip_flag

    return ts_com_map


ts_com_serv = threading.Thread(name='tscomserver', target=server)
ts_com_serv.start()
